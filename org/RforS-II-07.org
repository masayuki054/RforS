...
- formula, error model などに関する説明
  - [[https://stackoverflow.com/questions/37497948/aov-error-term-in-r-whats-the-difference-bw-errorid-and-errorid-timevar][anova - aov() error term in R: what's the difference bw Error(id) and Error(id/timevar) specification? - Stack Overflow]]
  - [[https://cran.r-project.org/doc/contrib/manuals-jp/Mase-Rstatman.pdf][Mase-Rstatman.pdf]] (線形回帰モデルの分散分析表)
...
- 交互作用効果 :: 2つ以上の要因が組み合わされたときに生じる効果
...
# 交互作用のある、なしに見られるデータの特徴:

# 以下ではinteraction.plotと
# いう、 交互作用効果を確認するための平均値のプロットを作成する関数を用
# いています:
...
# #+begin_src R :session *rfors* :results output graphics :file RforS/graph/07-03-01-1.png :exports both :tangle RforS/code/07.R

# data <- c(4,3,6,6,5,8)
# LvA <- rep(c("A1", "A2"),3)
# LvB <- rep(c("B1", "B2", "B3"),2)
# interaction.plot(LvA, LvB, data)

# #+end_src

# #+RESULTS:

# [[file:RforS/graph/07-03-01-1.png]]

# #+begin_src R :session *rfors* :results output graphics :file RforS/graph/07-03-01-2.png :exports both :tangle RforS/code/07.R

# data <- c(4,3,6,6,5,8)
# LvA <- rep(c("A1", "A2"),3)
# LvB <- rep(c("B1", "B2", "B3"),2)
# interaction.plot(LvB, LvA, data)

# #+end_src

# #+RESULTS:

# [[file:RforS/graph/07-03-01-2.png]]
...
# #+begin_src R :session *rfors* :results output graphics :file RforS/graph/07-03-02.png :exports both :tangle RforS/code/07.R
...
*** 二元配置分散分析 
    ...
    - 帰無仮説と対立仮説の設定
        ...
        - 要因A (温度の違い)の主効果
            - 帰無仮説 \(H_0\) ::
	      温度が違ってもおいしさ得点の母平均は等しい（要因Ａの主効果はない）
            - 対立仮説 \(H_1\) ::
	      温度の違いによっておいしさ得点の母平均は異なる（要因Ａの主効果がある）

        - 要因B (銘柄の違い) の主効果
            - 帰無仮説 \(H_0\) :: 銘柄が違ってもおいしさ得点の母平均は等しい(要因Bの主効果はない)
            - 対立仮説 \(H_1\) :: 銘柄の違いによっておいしさ得点の母平均は異なる(要因Bの主効果がある) 

        - 要因Aと要因Bの交互作用効果
            - 帰無仮説 \(H_0\) :: 温度と銘柄の組み合わせによるおいしさ
              得点の影響はない(要因Aと要因Bの交互作用効果はない)
            - 対立仮説 \(H_1\) :: 温度と銘柄の組み合わせによるおいしさ
              得点の影響がある(要因Aと要因Bの交互作用効果がある)

    - 検定統計量の選択 :: Fを検定統計量とする
    - 有意水準 \(\alpha\) の決定 ::  ここでは有意水準は5%、つまり \(\alpha=0.05\)とする。片側検定である
...
[[https://qiita.com/saka212/items/754635486c6be0e3100b][統計検定のための、Rの出力結果からわかること（分散分析編） - Qiita]]
[[https://qiita.com/saka212/items/8c74854171775e5d5273][統計検定のための、Rの出力結果からわかること（回帰分析編） - Qiita]]
    ...
    - 関数による交互作用効果の図:
      ...
      # interction.plot(横軸にとる要因、もう一方の要因、平均値を求める変数)


#+begin_src R :session *rfors* :results output graphics :file RforS/graph/07-03-1.png :exports both :tangle RforS/code/07.R
        interaction.plot(温度,銘柄,味)
#+end_src

#+RESULTS:

[[file:RforS/graph/07-03-1.png]]

    [温度ごとの銘柄と味との関係の図]
#+begin_src R :session *rfors* :results output graphics :file RforS/graph/07-03-2.png :exports both :tangle RforS/code/07.R    
    interaction.plot(銘柄,温度,味)
#+end_src

#+RESULTS:

[[file:RforS/graph/07-03-2.png]]
...
1. 二つの要因について同時に検討するだけでなく，

2. 残渣のばらつき(平方和) を減らせるので，
...
# 変数「味」, 「温度」, 「銘柄」 は前の例題のものを使う
 ...
 数字1D1 <- factor(rep(1:5,6))		# 評定者名の代わりに数字を使う場合

summary(aov(味~温度*銘柄			   # 対応のない場合と同じ
	+Error(人1+人1:温度+人1:銘柄+人1:温度:銘柄)))  # 個人差によるデータの分析

aov(味~温度*銘柄			   # 対応のない場合と同じ
	+Error(人1+人1:温度+人1:銘柄+人1:温度:銘柄))  # 個人差によるデータの分析
#+end_src
...
5. 帰無仮説の棄却or採択の決定:
  ...
  - 温度と銘柄の交互作用効果：5％水準で有意な効果がある（p=0.039)
    対応なしでは，(p=0.11515) で有意な効果はなかった
...
- 冷蔵庫と常温では異なる評定者が評定する
- 同じ温度に対しては同じ評定者が銘柄のそれぞれの要因の組合わせに対して評
  定を下す
...
# 変数「味」, 「温度」, 「銘柄」 は前の例題のものを使う
 ...
 数字ID2 <- factor(c(rep(1:5,3),rep(6:10,3)))		# 評定者名の代わりに数字を使う場合	
 summary(aov(味~温度*銘柄			# 対応のない場合と同じ
                  +Error(人2:温度+人2:温度:銘柄)))		# 追加されたオプション
#+end_src
