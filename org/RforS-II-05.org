...
#+begin_src R :session *rfors* :results output :exports both :tangle RforS/code/05-01.R
sample.size <- 1000
A <- rnorm(sample.size)
B <- rnorm(sample.size)
C <- A*0.5 + sqrt(0.75)*B
...
*** 標本
     ...
     #+begin_src R :session *rfors* :results output :exports both :tangle RforS/code/r-05-03.r
