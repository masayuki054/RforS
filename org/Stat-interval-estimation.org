...
* 平均パラメータの区間推定の問題
  ...
  20人の患者への投与により，増加の程度 (%) の標本値が得らた:

#+begin_src R :results output :session *stext* 
##
## 正規分布からの標本
##
### サイズ 20, 平均=29.2, 分散l=1
###
...
#+RESULTS:
:  [1] 29.26114 29.35022 30.98864 28.61105 29.52598 30.32363 29.52287 28.78283
:  [9] 29.77956 29.60103 29.93772 29.43573 28.41715 27.98047 27.94568 28.81546
: [17] 28.47134 30.37318 30.68485 29.76198
...
** 標本のの標本平均が入る区間を求める
   ...
   [[http://cse.naro.affrc.go.jp/takezawa/r-tips/r2.html][R-Tips]] 確率分布と乱数参照
...
**** 信頼度 0.95/0.99 の信頼区間は，
     ...
     #+begin_src R :tangle RforS/code/10.R
...
**** lt_{n-1}分布で信頼度(1-alpha)の信頼区間 (上記の説明通り)求める
...
*** \( \mu \) の信頼区間 (を求める公式) を求める
     ...
     \begin{eqnarray}
     1-\alpha & = & P (|T_n| \leq z^{*}) \\
     & = & P( \frac{|\overline{X_{(n)}}-\mu|}{\sqrt{S^2/n}} \leq  z^{*}_) \\
     & = & P( {|\overline{X_{(n)}}-\mu| \leq z^{*}_\sqrt{S^2/n}) \\
     & = & P \left( \overline{X_{(n)}}-z^{*} \sqrt{S^2/n} \leq \mu \leq  \overline{X_{(n)}} + z^{*}\sqrt{S^2/n} \right) \\
     \end{eqnarray}

     #+begin_src R :session *stext* :results output :exports both :tangle Rtext/code/01.R
...
** 薬と血中濃度の問題
   ...
   - Aグループ :: 新薬と投与し， \( \overline{X_{(n)}} = 3.56 \)
   - Bグループ :: 新薬と投与し， \( \overline{Y_{(m)}} = 1.27 \)

  平均的には，\( \( \overline{X_{(n)}} - \overline{Y_{(m)}} = 1.27 \)
  だが，その誤差の見積りは？
